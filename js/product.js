// Removed local hardcoded URL - now using PRODUCTS_API from api.js
const grid = document.getElementById("productsGrid");

// Get the user data and extract the email
  ...
  productsList.forEach((p, index) => {
    ...
    card.innerHTML = `
      <div class="aspect-square overflow-hidden bg-[#0a0c0e]">
        <img src="${p.image || p.imageUrl}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition duration-500">
      </div>
      <div class="p-6">
...
async function addToDatabase(index) {
  ...
  const productData = {
    ...
    price: p.price,
    imageUrl: p.image,
    ownerEmail: userEmail // Links product to the logged-in user
  };

  try {
    // Replaced local API_BASE_URL with the global variable from api.js
    ...
    if (response.ok) {
      ...
      // Update the local cart so it shows up on cart.html
    ...
    alert("Cannot connect to Java backend. Is IntelliJ running?");
  }
}
